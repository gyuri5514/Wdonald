		...
		<div class="row">
			...
			<div class="col-md-9">
				<h3 class="title-divider mt_0">
					<span>마이페이지</span> <small>MyPage</small>
				</h3>
								...
								<div class="col-xs-12">
									...
									<form method="post" role="form" data-form-sync="#form_consentnewnumber" class="form form-register" id="form_accountsetting" name="form_accountsetting" data-required-symbol="*" autocomplete="off">
										...
										<fieldset class="fieldset">
												...
												<div class="col-xs-12">
													<div class="form-group">
														<label class="field-label"
															for="form_accountsetting_firstname">성함:</label>
														<c:choose>
															<c:when test="${not empty sessionScope.userInfo}">
																<input type="text"
															...
															<c:when test="${not empty sessionScope.kakaoSession}">
																<input type="text"
																	class="form-control input-lg required no-special-characters"
																	id="form_accountsetting_firstname" name="user_name"
																	value="${sessionScope.kakaoSession.user_name }" />
															</c:when>
															<c:when test="${not empty sessionScope.naverSession}">
																<input type="text"
																	class="form-control input-lg required no-special-characters"
																	id="form_accountsetting_firstname" name="user_name"
																	value="${sessionScope.naverSession.user_name }" />
															</c:when>
														</c:choose>
													</div>
												</div>
											...
											<div class="row">
												<div class="col-xs-6">
													<div class="form-group">
														<label class="field-label"
															for="form_accountsetting_gender">성별:</label>
														<c:choose>
															<c:when test="${not empty sessionScope.userInfo}">
																<input type="text"
																	class="form-control input-lg required no-special-characters"
																	id="form_accountsetting_firstname" name="user_gender"
																	value="${sessionScope.userInfo.user_gender }" readonly />
															</c:when>
															<c:when test="${not empty sessionScope.kakaoSession}">
																<input type="text"
																	class="form-control input-lg required no-special-characters"
																	id="form_accountsetting_firstname" name="user_gender"
																	value="${sessionScope.kakaoSession.user_gender }"
																	readonly />
															</c:when>
															<c:when test="${not empty sessionScope.naverSession}">
																<input type="text"
																	class="form-control input-lg required no-special-characters"
																	id="form_accountsetting_firstname" name="user_gender"
																	value="${sessionScope.naverSession.user_gender }"
																	readonly />
															</c:when>
											...
											<div class="row">
												<div class="col-xs-12">
													<div class="form-group">
														<label class="field-label"
															for="form_accountsetting_contactno">휴대전화 번호:</label>
														<c:choose>
															<c:when test="${not empty sessionScope.userInfo}">
																<input type="text"
															...
															<c:when test="${not empty sessionScope.kakaoSession}">
																<input type="text"
																	...
																	maxlength="11" id="form_accountsetting_contactno"
																	name="user_phone" placeholder="숫자만 입력"
																	value="${sessionScope.kakaoSession.user_phone }" />
															</c:when>
															<c:when test="${not empty sessionScope.naverSession}">
																<input type="text"
																	class="form-control input-lg required number digit-only"
																	maxlength="11" id="form_accountsetting_contactno"
																	name="user_phone" placeholder="숫자만 입력"
																	value="${sessionScope.naverSession.user_phone }" />
															</c:when>
											...
											<div class="row">
												<div class="col-xs-12">
													<div class="form-group">
														<label for="m_password2">패스워드 확인</label> <input
															type="password" id="m_password2" value="" maxlength="16"
															class="form-control placeholder" /> <span
															id="pwd_chk_msg2" class="f11_ment"></span>
														...
														<div class="box-info mt_10" id="collapseExam">
															...
															<p>
																<small> 1) 비밀번호 작성 규칙은 3개 문자 조합으로 8자리 이상 가능합니다.<br />
																	2) 비밀번호 작성 규칙은 2개 문자 조합으로 10자리 이상 가능합니다.<br /> 3)
																	주민번호, 생일, 아이디, 전화번호, 연속된 숫자와 문자, <br /> 사전에 포함된 단어가
																	포함된 패스워드는 피해주시기 바랍니다.<br /> 4) 타 사이트와 동일한 패스워드의 사용,
																	이전에 사용했던<br /> 패스워드의 재사용은 안전하지 않을 수 있습니다.<br /> 5)
																	패스워드는 길수록, 그리고 많이 혼용할수록 안전합니다.<br />
																</small>
															</p>
															<h5>
															...
															<p class="last">
																<strong>- 사용불가</strong> : 보안에 매우 취약하여 사용할 수 없습니다.<br />
																<strong>- 낮음</strong> : 사용은 가능하지만 보안에 다소 취약합니다.<br /> <strong>-
																	적정</strong> : 사용 가능하며 적정수준의 안전한 패스워드입니다.<br /> <strong>-
																	높음</strong> : 매우 안전한 패스워드입니다.
															</p>
														</div>
													</div>
												</div>
										...
										<fieldset class="fieldset form-actions">
											<div class="form-group">
												<button type="button" class="btn btn-red btn-xl btn-submit"
													id="join_ok" onclick="updatego()">수정사항 저장</button>
												<c:choose>
													<c:when test="${not empty sessionScope.userInfo}">
																<a id="button" class="btn btn-red btn-xl btn-submit"
													href="mypageDelete.do?user_email=${sessionScope.userInfo.user_email}">회원
													탈퇴</a>
													</c:when>
													<c:when test="${not empty sessionScope.kakaoSession}">
															<a id="button" class="btn btn-red btn-xl btn-submit"
													href="mypageDelete.do?user_email=${sessionScope.kakaoSession.user_email}">회원
													탈퇴</a>
													</c:when>
													<c:when test="${not empty sessionScope.naverSession}">
																<a id="button" class="btn btn-red btn-xl btn-submit"
													href="mypageDelete.do?user_email=${sessionScope.naverSession.user_email}">회원
													탈퇴</a>
													</c:when>
			...
			<script type="text/javascript">
				function updatego() {
					...
					if (($("#m_password").val()).trim() == "") {
						...
						confirm("변경하시겠습니까?");
						var mypageUpdateForm = document
								.getElementById("form_accountsetting");
